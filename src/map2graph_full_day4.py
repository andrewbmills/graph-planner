...
class node_skeleton:
	...
	def findFrontierNodes(self, nodes, img):
		...
		# Find all pixels not adjacent to occupied pixels
		occ_conv = ndimage.convolve(img_occ, kernel, mode='constant', cval=0)
		img_occ_adjacent_not = np.equal(occ_conv, 0)
		frontier = frontier*img_occ_adjacent_not

		# Cluster frontiers
		connected_thresh = self.frontier_cluster_thresh
		frontier, num_labels = measure.label(frontier, connectivity=2, return_num=True)
		# print("number of frontier cluster labels: %d" % (num_labels))
		# print("largest label in frontier img %d" % np.max(frontier))

		cluster_sizes = []
		cluster_sizes_filtered = []
		for label in range(1,num_labels+1):
			cluster = np.equal(frontier, label)
			count = np.sum(cluster)
			if (count < connected_thresh):
				# print(np.sum(cluster))
				frontier[cluster] = 0
				cluster_sizes_filtered.append([count, label])
			else:
				# print(count)
				cluster_sizes.append([count, label])
		...
		# 	cluster_counts.append(np.sum(cluster))
			
		# connected_thresh = min(np.max(cluster_counts), self.frontier_cluster_thresh)
		# for i in range(len(cluster_counts)):
		# 	if cluster_counts[i] < connected_thresh:
		# 		frontier[clusters[i]] = 0

		frontier = np.greater(frontier,0.1)

		# plt.bar(np.arange(len(cluster_sizes)), np.sort(cluster_sizes))
		# plt.draw()
		# plt.pause(5.0)
		...
		for node in nodes:
			...
			# plt.draw()
			# plt.pause(2.0)
			# print(np.sum(neighborhood))
			if np.sum(neighborhood) > self.frontier_neighbor_thresh:
		...
		if (self.map_get):
			...
			for node in nodes_all:
				if (node_num in frontier_node_indices) and not (node_num == home_node):
					...
					newNode = Node()
					# print("Adding new node. node_ids = %d" % (node_ids))
					if (node_num == home_node):
						newNode.id = 0
					else:
						newNode.id = node_ids
						node_ids = node_ids + 1
					# print("newNode.id = %d and node_ids = %d" % (newNode.id, node_ids))
					...
					if newNode.id == 0:
						# print("home node inserted")
						self.graph_msg.node.insert(0, newNode)
					else:
						# print("node appended to graph msg.")
			...
			# self.graph_msg.size = len(self.graph_msg.node)
			self.graph_msg.size = self.graph_msg.node[-1].id + 1

			# Renumber node ids to make everything consistent
			# id_count = 0
			# renumber_ids = [0]*(self.graph_msg.node[-1].id + 1)
			# for node in self.graph_msg.node:
			# 	renumber_ids[node.id] = id_count
			# 	node.id = id_count
			# 	id_count = id_count + 1

			# for node in self.graph_msg.node:
			# 	for neighbor in node.neighborId:
			# 		neighbor = renumber_ids[neighbor]

			if (self.time_msgs):
				print("--- %0.2f ms: Frontier and msg formatting ---" % ((time.time() - start_time)*1000.0))
	...
	def __init__(self):
		...
		self.frontier_neighbor_thresh = float(rospy.get_param("map2graph/frontier_neighbor_thresh", 50.0))
