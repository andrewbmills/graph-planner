...
class graph2path:
	...
	def findPath(self):
		...
		if (self.task == "Home"):
			...
			if (len(goal_list)>0):
				dist = np.array(dist)
				if self.goal_biasing:
					# y-average goal biasing
					goal_positions_y = []
					for ind in goal_list:
						goal_positions_y.append(self.node_position_list[ind].y)
					goal_positions_y = np.array(goal_positions_y)
					y_average = np.mean(goal_positions_y)
					goal_list_y_filtered = []
					goal_list_count = 0
					edge_idx_table = []
					for goal in goal_list:
						if (self.node_position_list[goal].y <= (y_average + 0.1)) and (self.goal_bias_direction == "right"): # right biased case
							goal_list_y_filtered.append(goal)
							edge_idx_table.append(goal_list_count)
						if (self.node_position_list[goal].y >= (y_average - 0.1)) and (self.goal_bias_direction == "left"): # left biased case
							goal_list_y_filtered.append(goal)
							edge_idx_table.append(goal_list_count)
						goal_list_count = goal_list_count + 1
					goal_list_y_filtered = np.array(goal_list_y_filtered)
					destination_edge_idx = edge_idx_table[np.argmin(dist[goal_list_y_filtered])]
					goal_id = goal_list[destination_edge_idx]
				else:
					# Original Behavior (Go to closest unexplored edge)
		...
		if (len(node_list) > 1):
			...
			if (self.task == "Home") or (goal_id == 0):
				...
				if self.turn_biasing:
					bias_angle = np.arctan2(self.turn_goal_y - self.node_position_list[node_list[-1]].y, self.turn_goal_x - self.node_position_list[node_list[-1]].x)
	...
	def __init__(self):
		...
		self.goal_biasing = bool(rospy.get_param("graph2path/goal_biasing", False))
		self.goal_bias_direction = str(rospy.get_param("graph2path/goal_bias_direction", "left"))
